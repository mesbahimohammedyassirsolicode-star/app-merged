...
/**
 * Seeds the official TSGE 1A timetable (EMPLOI RAMADAN FD 2024-2025).
 * Filière: TSGE, Group: 1A, Academic Year: 2024-2025, Session 2, Matin & Soir.
 */
class Tsge1ATimetableSeeder extends Seeder
{
    protected string $dataPath = 'database/data/tsge_1a_emploi_2024_2025_s2.json';
    ...
    public function run(): void
    {
        ...
        $this->command->info('Seeding TSGE 1A timetable (2024-2025 Session 2)...');
        ...
        $this->command->info('TSGE 1A timetable seeded successfully.');
    }

    private function ensureAnneeScolaire(): AnneeScolaire
    {
        // Use current academic year if set (e.g. by DemoDataSeeder), so TSGE-1A appears with other groups
    ...
    /** @return array<string, Formateur> key = teacher display name from PDF */
    private function ensureFormateurs(array $seances): array
    {
        ...
        foreach ($names as $displayName) {
            ...
            $email = $slug.'@gims.ma';
            $name = trim(preg_replace('/^\s*M\.?\s*/i', '', $displayName));
    ...
    /** @return array<string, Affectation> key = "module_code|teacher" */
