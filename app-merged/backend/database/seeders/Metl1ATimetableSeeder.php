...
/**
 * Seeds the Mastère Européen Transport et Logistique (METL) 1A timetable.
 * Filière: METL, Group: 1A, Academic Year: 2024-2025, Session 2.
 * Template based on program brochure - update JSON with official emploi du temps when available.
 */
class Metl1ATimetableSeeder extends Seeder
{
    protected string $dataPath = 'database/data/metl_1a_emploi_2024_2025_s2.json';
    ...
    public function run(): void
    {
        ...
        $this->command->info('Seeding METL 1A timetable (2024-2025 Session 2)...');
        ...
        DB::transaction(function () use ($data) {
            ...
            $this->ensureAssignments($data['seances'], $group, $modules, $formateurs);
            $this->generateSeances($data['seances'], $group, $modules, $formateurs, $annee);
        });

        $this->command->info('METL 1A timetable seeded successfully.');
    }
    ...
    private function ensureFiliere(Niveau $niveau, array $meta): Filiere
    {
        return Filiere::firstOrCreate(
            ...
            [
                ...
                'description' => 'Mastère Européen Transport et Logistique - Session 2',
            ]
        );
    }
    ...
    private function ensureFormateurs(array $seances): array
    {
        ...
        foreach ($names as $displayName) {
            ...
            $formateurs[$displayName] = Formateur::firstOrCreate(
                ['user_id' => $user->id],
                [
                    'matricule' => 'F_METL_'.$user->id,
                    'specialty' => 'Transport et Logistique',
                    'type' => 'permanent',
                ]
            );
        }
    ...
    private function ensureAssignments(
        array $seances,
        Groupe $group,
        array $modules,
        array $formateurs
        ...
        foreach ($seances as $s) {
            ...
            if (! isset($modules[$code]) || ! isset($formateurs[$teacherName])) {
                continue;
            }
            $formateur = $formateurs[$teacherName];
            $user = $formateur->user;

            // Assign module and group to user
            $user->modules()->syncWithoutDetaching([$modules[$code]->id]);
            $user->groups()->syncWithoutDetaching([$group->id]);
        }
    }

    private function generateSeances(array $seances, Groupe $group, array $modules, array $formateurs, AnneeScolaire $annee): void
    {
        $dayOffset = [
            'Monday' => 0,
            'Tuesday' => 1,
        ...
        for ($w = 0; $w < $weeks; $w++) {
            ...
            foreach ($seances as $s) {
                ...
                $module = $modules[$s['module_code']] ?? null;
                $formateur = $formateurs[$s['teacher']] ?? null;
