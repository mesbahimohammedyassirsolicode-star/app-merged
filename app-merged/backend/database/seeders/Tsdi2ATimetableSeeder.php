...
/**
 * Seeds the official TSDI 2A timetable (EMPLOI RAMADAN TSDI 2A).
 * Filière: TSDI, Group: 2A, Academic Year: 2025-2026, Session 2, Matin & Soir.
 */
class Tsdi2ATimetableSeeder extends Seeder
{
    protected string $dataPath = 'database/data/tsdi_2a_emploi_2025_2026_s2.json';
    ...
    public function run(): void
    {
        ...
        $this->command->info('Seeding TSDI 2A timetable (2025-2026 Session 2)...');
        ...
        $this->command->info('TSDI 2A timetable seeded successfully.');
    }
    ...
    private function ensureFormateurs(array $seances): array
    {
        ...
        foreach ($names as $displayName) {
            $slug = strtolower(preg_replace('/[^a-z0-9]+/i', '_', trim(str_replace(['M.', 'Mme.', 'M ', 'Formateur', 'Formatrice'], '', $displayName))));
            $email = $slug.'@gims.ma';
            $name = trim(preg_replace('/^\s*(M\.|Mme\.?|M\s+|Formateur\s*|Formatrice\s*)\s*/i', '', $displayName));
            ...
            $formateurs[$displayName] = Formateur::firstOrCreate(
                ['user_id' => $user->id],
                [
                    'matricule' => 'F_TSDI_'.$user->id,
                    'specialty' => $name ?: $displayName,
                    'type' => 'permanent',
                ]
            );
        }
