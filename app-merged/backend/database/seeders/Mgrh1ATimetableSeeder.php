...
/**
 * Seeds the Mastère Européen Management des Ressources Humaines (MGRH) 1A timetable.
 * Filière: MGRH, Group: 1A, Academic Year: 2024-2025, Session 2.
 * Template based on program brochure - update JSON with official emploi du temps when available.
 */
class Mgrh1ATimetableSeeder extends Seeder
{
    protected string $dataPath = 'database/data/mgrh_1a_emploi_2024_2025_s2.json';
    ...
    public function run(): void
    {
        ...
        $this->command->info('Seeding MGRH 1A timetable (2024-2025 Session 2)...');
        ...
        $this->command->info('MGRH 1A timetable seeded successfully.');
    }
    ...
    private function ensureFiliere(Niveau $niveau, array $meta): Filiere
    {
        return Filiere::firstOrCreate(
            ...
            [
                ...
                'description' => 'Mastère Européen Management des Ressources Humaines - Session 2',
            ]
        );
    }
    ...
    private function ensureFormateurs(array $seances): array
    {
        ...
        foreach ($names as $displayName) {
            ...
            $formateurs[$displayName] = Formateur::firstOrCreate(
                ['user_id' => $user->id],
                [
                    'matricule' => 'F_MGRH_'.$user->id,
                    'specialty' => 'Management des Ressources Humaines',
                    'type' => 'permanent',
                ]
            );
        }
